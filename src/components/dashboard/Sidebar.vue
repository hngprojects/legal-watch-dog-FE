    ...
    <nav class="flex-1 p-4">
      <ul class="space-y-2">
        <li v-for="item in menuItems" :key="item.path">
          <router-link :to="item.path" class="flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors"
            :class="isActive(item.path) ? 'bg-[#3C2610] text-white' : 'text-gray-700 hover:bg-amber-100'">
            <component :is="item.icon === 'HomeIcon' ? HomeIcon :
              item.icon === 'ProjectsIcon' ? ProjectsIcon :
                item.icon === 'JurisdictionsIcon' ? JurisdictionsIcon : HomeIcon" class="w-5 h-5" />
            <span>{{ item.name }}</span>
          </router-link>
        </li>
      </ul>
