...
<template>
  <main class="min-h-screen flex-1 bg-gray-50 p-6 lg:p-10">
    ...
    <div v-else-if="!project" class="mx-auto max-w-7xl py-16 text-center">
      ...
      <button @click="goBack" class="text-[#401903] hover:underline cursor-pointer">
    ...
    <div v-else class="mx-auto max-w-7xl">
      <div class="mb-8 flex items-center justify-between">
        ...
        <div class="relative">
          <button @click.stop="toggleSettingsMenu"
            class="flex h-10 w-10 cursor-pointer items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
            <Settings :size="18" />
          </button>

          <div v-if="showSettingsMenu" @click.stop
            class="absolute right-0 mt-2 w-44 rounded-md bg-white shadow-lg ring-1 ring-black/5 z-50">
            <button @click="startEdit" class="w-full text-left px-4 py-2 text-sm hover:bg-gray-50">
              Edit Project
            </button>
            <button @click="deleteProject" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50">
              Delete Project
            </button>
          </div>
        </div>
      ...
      <div class="mb-8 rounded-[10px] bg-white p-5 flex flex-col gap-5">
        <template v-if="showInlineEdit">
          <form @submit.prevent="saveEdit" class="space-y-4 w-full">
            <div>
      ...
      <div class="mb-8 flex items-end justify-between border-b border-gray-200 md:mt-[88px]">
        <div class="flex gap-8">
          ...
          <button @click="activeTab = 'activity'" :class="[
            ...
            Activity
            <div v-if="activeTab === 'activity'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-[#401903]"></div>
          </button>
        </div>
